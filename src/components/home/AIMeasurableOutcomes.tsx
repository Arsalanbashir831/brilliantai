  ...
  return (
    <div
      className="
        ...
        w-auto md:w-full h-full md:h-96 overflow-hidden flex-wrap
      ">
      <ShineBorder shineColor={["#23D5D5", "#00FFFF"]} />

      <div>
        <h3 className="text-lg font-light md:text-2xl md:font-medium">
          {title}
        </h3>
      ...
      <div className="flex gap-1 items-start md:items-center mt-6 text-xs md:text-base text-white">
        <Image
...
const tabs = [
  {
    ...
    cardProps: {
      ...
      imageClass: "top-38 right-14 w-96",
      footerText:
  ...
  },
  {
    ...
    cardProps: {
      ...
      imageClass: "top-4 right-10 w-96",
  ...
  return (
    <section className="bg-[#011010] text-white py-20  md:px-0 lg:px-40">
      ...
      <div className="max-w-8xl mx-auto text-center mb-12 px-6">
        <h2
          style={{ fontSize: !isMobile ? "50px" : "30px" }}
          className="font-bold">
          AI That Drive measurable outcomes
        </h2>
        <p className="mt-1 text-white/80 text-base sm:text-lg max-w-3xl mx-auto">
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-5 md:justify-center  pt-8">
        {/* Sidebar */}
        {!isMobile && (
          <ul className="space-y-6  pl-8 relative w-[33%] ">
            {tabs.map(({ key, label }) => (
              <li
                key={key}
                onClick={() => setActiveKey(key)}
                className="relative cursor-pointer pl-4 text-md transition-colors">
                ...
                <span
                  className={`relative text-[20px] font-light ${
                    key === activeKey
        ...
        <div
          ...
          {isMobile ? (
            tabs.map(({ key, cardProps }) => (
              <motion.div
                key={key}
                className="md:min-w-[80%] w-[85%] shrink-0"
