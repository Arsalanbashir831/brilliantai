import Image from "next/image";

const stats = [
  { value: "130+", label: "AI solutions built" },
  { value: "40%", label: "Year-over-year team growth" },
  { value: "5", label: "Successful product exits" },
];

export function StatsSection() {
  return (
    <section className="w-full px-4 md:px-6 py-16 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            <h2 className="text-3xl md:text-5xl md:font-bold font-light text-white mb-6">
              Empowering Innovation
            </h2>
            <p className="text-gray-300 text-lg leading-relaxed">
              From MVPs to enterprise AI systems, we help founders, product
              teams, and public sector organisations bring their AI visions to
              life with clarity and execution.
            </p>
          </div>
          <div className="flex justify-center w-full">
            <div className="relative w-full md:max-w-full flex flex-col items-center justify-center">
              {/* Glow Behind */}
              <div className="absolute top-[-20%] right-[-20%] w-[140%] h-[140%] z-0 pointer-events-none">
                <Image
                  src="/about/story.svg"
                  alt=""
                  fill
                  className="object-contain opacity-60 blur-2xl"
                />
              </div>

              {/* Card Content */}
              <div className="relative z-10 w-full bg-[#050A0B]/70 border border-white/10 rounded-[10px] overflow-hidden">
                <div className="flex flex-col">
                  {stats.map((stat, idx) => (
                    <div key={stat.value}>
                      <div className="px-10 py-5 flex flex-col">
                        <span className="text-4xl font-normal text-white">
                          {stat.value}
                        </span>
                        <span className="mt-1 text-md leading-[28px] font-normal text-[#E0E0E0]">
                          {stat.label}
                        </span>
                      </div>
                      {idx < stats.length - 1 && (
                        <div className="h-[3px] w-full bg-gradient-to-r from-[#00AEFF] via-[#00DE94] to-[#00FF52] opacity-30" />
                      )}
                    </div>
                  ))}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
